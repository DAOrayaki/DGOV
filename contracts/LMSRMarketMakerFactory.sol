...
contract LMSRMarketMakerData {
    ...
    YAKIPoint internal collateralToken;
...
contract LMSRMarketMakerFactory is ConstructedCloneFactory, LMSRMarketMakerData {
    event LMSRMarketMakerCreation(address indexed creator, LMSRMarketMaker lmsrMarketMaker, ConditionalTokens pmSystem, YAKIPoint collateralToken, bytes32[] conditionIds, uint64 fee, uint funding);
    ...
    function cloneConstructor(bytes calldata consData) external {
        (
            ...
            YAKIPoint _collateralToken,
            bytes32[] memory _conditionIds,
            uint64 _fee,
            Whitelist _whitelist
        ) = abi.decode(consData, (ConditionalTokens, YAKIPoint, bytes32[], uint64, Whitelist));
    ...
    function createLMSRMarketMaker(ConditionalTokens pmSystem, YAKIPoint collateralToken, bytes32[] calldata conditionIds, uint64 fee, Whitelist whitelist, uint funding)
        external
        returns (LMSRMarketMaker lmsrMarketMaker)
    {
        ...
        collateralToken.transferPointFrom(msg.sender, address(this), funding);
        collateralToken.approvePoint(address(lmsrMarketMaker), funding);
