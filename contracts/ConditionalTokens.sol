...
contract ConditionalTokens is ERC1155 {
    ...
    event PayoutRedemption(
        ...
        YAKIPoint indexed collateralToken,
        bytes32 indexed parentCollectionId,
        bytes32 conditionId,
        uint[] indexSets,
        uint payout
    ...
    function splitPosition(
        ...
        if (freeIndexSet == 0) {
            ...
            if (parentCollectionId == bytes32(0)) {
                require(collateralToken.transferPointFrom(msg.sender, address(this), amount), "could not receive collateral tokens");
            } else {
                _burn(
                    msg.sender,
    ...
    function mergePositions(
        ...
        if (freeIndexSet == 0) {
            if (parentCollectionId == bytes32(0)) {
                require(collateralToken.transferPoint(msg.sender, amount), "could not send collateral tokens");
            } else {
                _mint(
                    msg.sender,
                    CTHelpers.getPositionId(collateralToken, parentCollectionId),
    ...
    function redeemPositions(YAKIPoint collateralToken, bytes32 parentCollectionId, bytes32 conditionId, uint[] calldata indexSets) external {
        ...
        if (totalPayout > 0) {
            if (parentCollectionId == bytes32(0)) {
                require(collateralToken.transferPoint(msg.sender, totalPayout), "could not transfer payout to message sender");
            } else {
    ...
    function getPositionId(YAKIPoint collateralToken, bytes32 collectionId) external pure returns (uint) {
